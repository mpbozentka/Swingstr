...
const MenuButton = ({ icon: Icon, label, active, onClick, isOpen }) => (
  <button
    onClick={onClick}
    className={`relative flex items-center gap-2 px-3 py-2 rounded-lg transition-all ${active || isOpen ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-gray-200'
      }`}
  >
...
export default function Swingstr() {
  ...
  const [students, setStudents] = useState([{ id: 1, name: "Demo Student", email: "demo@golf.com", phone: "555-0123", videos: [], notes: "Working on takeaway path." }]);
  ...
  const handleClearVideo = (side) => {
    ...
    // Reset global controls if both are cleared or if active is cleared
  ...
  const handleTimeUpdate = (time, dur) => {
    // Only update global time if it comes from the active screen
    setGlobalTime(time);
